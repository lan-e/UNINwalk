...
export async function getChatbotAnswer(question, t = (key) => key) {
  // Auto-initialize if not initialized
  if (!chatChain || !initializationStatus.isInitialized) {
    console.log("Chatbot not initialized, initializing now...");
    const status = await initializeChatbot(t);
    
    if (!status.isInitialized || status.error) {
      throw new Error(status.error || "Failed to initialize chatbot");
    }
  }
