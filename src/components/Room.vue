<template>
    <g :id="'room-' + room.id" @click="handleRoomClick"
        ...
        <rect :x="room.x" :y="room.y" :width="room.width" :height="room.height" class="room-color" />
        <text :x="room.x + room.width / 2" :y="room.y + 30" class="room-name-color" font-weight="bold" text-anchor="middle">
            {{ room.name }}
        </text>
        <foreignObject
            v-if="room.type"
            ...
            class="room-name room-name-color"
        >
            {{ room.type }}
        </foreignObject>
        <ToiletsIcon v-if="room.id.includes('toilets')" :x="room.x + 14" :y="room.y" class="toilets-color" />
